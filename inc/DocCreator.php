...
use NCL\Core\NCL;//Библиотека склонения ФИО по падежам
...
//==============================================================================
    ...
    function copyRange( Worksheet $sheet, $srcRange, $dstCell) {
        // Проверяем корректность диапазона. Например: A2:A3, A2:AB2, A27:B100
        if( !preg_match('/^([A-Z]+)(\d+):([A-Z]+)(\d+)$/', $srcRange, $srcRangeMatch) ) {
            return false;
        }
        // Проверяем корректность адреса конечной ячейки. Например: A2, AB3, A27
        ...
        // Преобразуем индексы для циклов
        // Вычитаем 1, так как столбец начинается с 0, а не на 1.
        ...
        for ($row = $srcRowStart; $row <= $srcRowEnd; $row++) {
            ...
            for ($col = $srcColumnStart; $col <= $srcColumnEnd; $col++) {
                ...
                // Устанавливаем ширину столбца,  только один раз для каждой строки
                if ($rowCount === 0) {
